  ...
  return (
    <>
      ...
      <button
        ...
        className="group relative inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-gradient-to-r from-[#0D3827] via-[#08261A] to-[#04160E] border border-amber-500/35 hover:border-amber-400/60 shadow-[0_2px_12px_rgba(212,175,55,0.15)] transition-all cursor-pointer text-left"
      >
        {/* Subtle crescent minaret glow aura */}
        <div className="absolute -inset-0.5 bg-gradient-to-r from-amber-500/20 to-emerald-600/20 rounded-full blur-xs opacity-60 group-hover:opacity-100 transition-opacity pointer-events-none" />
        ...
        <div className="relative z-10 flex items-baseline gap-1.5">
          ...
          <span className="text-[9px] text-emerald-300/80 font-metric font-mono">
          ...
          <span
            className={`text-[9px] font-metric font-semibold px-1.5 py-0.2 rounded-full border ${
              upcomingPrayer.isApproachingSoon
                ...
                : 'bg-[#041E14] text-emerald-200/90 border-amber-500/30'
            }`}
          >
      ...
      {showScheduleModal && (
        ...
        >
          <div
            className="w-full max-w-sm rounded-3xl bg-gradient-to-b from-[#0D3827] via-[#072418] to-[#04140E] border border-amber-500/40 p-5 shadow-2xl relative overflow-hidden text-white animate-in fade-in zoom-in-95 duration-150"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Ambient Mosque Sky Glow */}
            <div className="absolute top-0 right-0 w-48 h-48 bg-amber-500/10 rounded-full blur-3xl pointer-events-none" />
            <div className="absolute bottom-0 left-0 w-48 h-48 bg-emerald-700/15 rounded-full blur-3xl pointer-events-none" />

            {/* Header */}
            <div className="flex items-center justify-between relative z-10 mb-3 border-b border-amber-500/20 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-sm font-bold tracking-tight text-amber-50 flex items-center gap-1.5">
                    <span>Midnight Mosque Prayer Times</span>
                  </h3>
                  <p className="text-[10px] text-amber-300/90 font-metric">
              ...
              <button
                onClick={() => setShowScheduleModal(false)}
                type="button"
                className="w-7 h-7 rounded-full bg-[#061D14] hover:bg-[#0B3322] border border-amber-500/30 text-amber-300 hover:text-white flex items-center justify-center transition-colors cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="mb-4 p-3.5 rounded-2xl bg-gradient-to-r from-amber-950/40 via-[#0B3322] to-emerald-950/50 border border-amber-500/35 relative z-10 flex items-center justify-between">
              <div>
                <span className="text-[9px] uppercase font-metric tracking-widest text-amber-400 font-bold block">
                  Next Congregation
                </span>
                <div className="flex items-baseline gap-2 mt-0.5">
                  <span className="text-base font-extrabold text-amber-50 font-metric">
                    {upcomingPrayer.name}
                  </span>
                ...
                <span className="text-xs text-emerald-200/90 font-metric mt-0.5 block">
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-between px-3 py-2 rounded-xl transition-all ${
                      isNext
                        ? 'bg-[#0B3824] border border-amber-400/60 text-amber-100 shadow-sm'
                        : 'bg-[#061D14] border border-amber-500/20 text-emerald-200/80 hover:text-amber-100'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      <Clock
                        className={`w-3.5 h-3.5 ${
                          isNext ? 'text-amber-400' : 'text-emerald-400/60'
                        }`}
                      />
                      <span className="text-xs font-metric font-semibold">{p.name}</span>
                      <span className="text-[11px] font-arabic text-emerald-300/70">{p.ar}</span>
                    </div>

                    <div className="flex items-center gap-2 font-mono text-xs">
                      <span className={isNext ? 'text-amber-300 font-bold' : 'text-emerald-200'}>
                        {time}
                      </span>
            ...
            {/* Refresh Footer */}
            <div className="mt-4 pt-3 border-t border-amber-500/20 flex items-center justify-between text-[10px] text-emerald-300/70 font-metric relative z-10">
