...
export const TaqwaProgressionPattern: React.FC = () => {
  ...
  const renderMilestoneIcon = (days: number, isUnlocked: boolean, isActive: boolean) => {
    const iconClass = isUnlocked
      ? 'text-amber-300 drop-shadow-[0_0_8px_rgba(212,175,55,0.8)]'
      : isActive
      ? 'text-emerald-200'
      : 'text-emerald-400/50';
    ...
    switch (days) {
      ...
      case 21:
        return <Flame className={`w-5 h-5 ${iconClass} fill-amber-400/40`} />;
      case 30:
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-amber-50 tracking-tight">
            Journey Progression
          </h3>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="flex items-center p-0.5 rounded-full bg-[#061D14] border border-amber-500/30 text-[10px] font-metric font-semibold">
            <button
          ...
          <button
            ...
            className={`p-1.5 rounded-full border transition-all cursor-pointer ${
              showSimulator
                ...
                : 'bg-[#061D14] border-amber-500/30 text-amber-300 hover:text-white'
            }`}
            type="button"
          >
      ...
      {showSimulator && (
        <div className="p-3 rounded-2xl bg-[#08261A] border border-amber-500/30 space-y-2 animate-fadeIn shadow-lg">
          <div className="flex items-center justify-between text-[11px]">
            <div className="flex items-center gap-1.5 text-amber-300 font-semibold">
              <Zap className="w-3.5 h-3.5 text-amber-400" />
              <span>Taqwa Streak Progression Simulator</span>
            </div>
            <span className="text-amber-300 font-metric font-bold">
          ...
          <div className="flex flex-wrap gap-1.5 pt-1">
            <button
              ...
              className="px-2.5 py-1 rounded-lg bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/40 text-emerald-300 text-[10px] font-bold font-metric transition-colors cursor-pointer"
              type="button"
            >
              +1 Day
            </button>
            {[1, 3, 7, 14, 21, 30, 40, 100].map((d) => (
              <button
                ...
                className={`px-2 py-1 rounded-lg text-[10px] font-metric font-semibold border transition-all cursor-pointer ${
                  currentStreak === d
                    ? 'bg-amber-400 text-black border-amber-400 shadow-md font-bold'
                    : 'bg-[#061D14] hover:bg-[#0B3322] border-amber-500/25 text-emerald-200'
                }`}
                type="button"
              >
                {d}d
            ...
            <button
              ...
              className="px-2 py-1 rounded-lg bg-[#04140E] hover:bg-[#061D14] text-zinc-300 text-[10px] font-metric border border-amber-500/20 cursor-pointer"
              type="button"
            >
              Reset 0d
            </button>
      ...
      <div className="relative rounded-3xl bg-gradient-to-b from-[#0D3827] via-[#08261A] to-[#04140E] border border-amber-500/35 p-5 sm:p-6 overflow-hidden shadow-[0_12px_40px_rgba(4,20,14,0.6)]">
        {/* Subtle background stars / nebula glow */}
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-64 h-64 bg-emerald-600/10 rounded-full blur-3xl pointer-events-none"></div>
              ...
              return (
                ...
                >
                  ...
                  {isActiveTarget && !isUnlocked && (
                    <div className="absolute -inset-3 bg-emerald-500/35 rounded-full blur-md animate-pulse pointer-events-none"></div>
                  )}
                  ...
                  <div
                    className={`relative w-14 h-14 rounded-2xl flex flex-col items-center justify-center transition-all duration-300 shadow-xl ${
                      isUnlocked
                        ? 'bg-gradient-to-b from-[#0D3F2A] to-[#061D14] border-2 border-amber-400 shadow-[0_0_20px_rgba(212,175,55,0.5)] scale-105'
                        : isActiveTarget
                        ? 'bg-gradient-to-b from-[#0B3824] to-[#04160E] border-2 border-amber-400 shadow-[0_0_18px_rgba(212,175,55,0.4)] ring-4 ring-emerald-500/30'
                        : 'bg-[#061D14] border border-amber-500/20 opacity-70 hover:opacity-100'
                    } ${isSelected ? 'ring-2 ring-white ring-offset-2 ring-offset-[#04140E]' : ''}`}
                  >
                    {/* Milestone Icon */}
                    ...
                    <span
                      className={`text-[10px] font-metric font-extrabold leading-none mt-0.5 ${
                        isUnlocked ? 'text-amber-300' : 'text-emerald-300/80'
                      }`}
                    >
                      {node.days}d
                    </span>
                    ...
                    <div className="absolute -top-1.5 -right-1.5">
                      {isUnlocked ? (
                        ...
                        <div className="w-5 h-5 rounded-full bg-emerald-600 text-white flex items-center justify-center shadow-md font-bold text-[9px]">
                          ⚡
                        </div>
                      ) : (
                        <div className="w-4 h-4 rounded-full bg-[#04140E] border border-amber-500/30 text-amber-400/40 flex items-center justify-center">
                  ...
                  <div
                    className={`absolute top-full mt-1.5 whitespace-nowrap -translate-x-1/2 left-1/2 px-2 py-0.5 rounded-full text-[9px] font-semibold transition-all ${
                      isUnlocked
                        ? 'bg-[#0B3824] text-amber-300 border border-amber-400/40 shadow-sm'
                        : isActiveTarget
                        ? 'bg-emerald-600 text-white shadow-[0_0_10px_rgba(16,185,129,0.6)] font-bold'
                        : 'bg-[#061D14]/90 text-emerald-200/80 border border-amber-500/20'
                    }`}
                  >
                    {milestone.name}
                  </div>
        ...
        {viewMode === 'ring' && (
          <div className="relative w-full max-w-[290px] mx-auto py-6 flex items-center justify-center">
            ...
            {/* Central Sun Core */}
            <div className="absolute inset-0 m-auto w-24 h-24 rounded-full bg-gradient-to-br from-[#0B3824] to-[#04160E] border-2 border-amber-400/40 flex flex-col items-center justify-center text-center shadow-[0_0_30px_rgba(212,175,55,0.4)] z-0">
              <span className="text-[9px] uppercase font-metric tracking-widest text-amber-300 font-bold">
                TAQWA
              </span>
              <span className="font-metric text-3xl font-black text-amber-50 leading-none my-0.5">
                {currentStreak}
              </span>
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${
                      isUnlocked
                        ? 'bg-[#0D3F2A] border-2 border-amber-400 text-amber-300 shadow-[0_0_15px_rgba(212,175,55,0.6)] scale-110'
                        : 'bg-[#061D14] border border-amber-500/20 text-emerald-300/40 hover:border-amber-500/40'
                    } ${isSelected ? 'ring-2 ring-white' : ''}`}
                  >
                    {renderMilestoneIcon(m.days, isUnlocked, false)}
                  </div>
                  <span className="absolute -bottom-4 left-1/2 -translate-x-1/2 text-[9px] font-metric font-bold text-emerald-200">
                    {m.days}d
                  </span>
                </div>
              );
        ...
        <div className="mt-5 p-4 rounded-2xl bg-[#061D14] border border-amber-500/30 relative overflow-hidden shadow-inner">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-2.5">
              <div className="w-10 h-10 rounded-xl bg-[#08261A] border border-amber-500/40 flex items-center justify-center text-lg shrink-0">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-metric font-semibold text-amber-300 bg-[#04160E] px-2 py-0.5 rounded-full border border-amber-500/30">
            ...
            <div className="shrink-0 text-right">
              {currentStreak >= selectedInfo.days ? (
                ...
                <span className="text-[10px] font-metric font-semibold text-amber-300/80">
          ...
          <div className="mt-3 pt-3 border-t border-amber-500/20 space-y-2">
            <div className="flex items-start gap-2">
              ...
              <p className="text-xs text-emerald-100/95 leading-relaxed italic">
                “{extraInfo.hadithOrAyah}”
                <span className="text-[10px] text-amber-300/80 not-italic block mt-0.5 font-metric">
                  — {extraInfo.source}
                </span>
              </p>
            </div>

            <div className="flex items-start gap-2 bg-[#08261A] p-2.5 rounded-xl border border-amber-500/25 text-[11px] text-emerald-200">
