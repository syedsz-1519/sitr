...
export const TaqwaStreakView: React.FC = () => {
  ...
  const handleSealNiyyah = () => {
    ...
    confetti({
      ...
      spread: 70,
      origin: { y: 0.55 },
      colors: ['#10B981', '#D4AF37', '#059669', '#FDE68A'],
    });
  ...
  const renderMilestoneIcon = (days: number) => {
    switch (days) {
      ...
      case 40:
        return <Diamond className="w-5 h-5 text-emerald-300" />;
      case 100:
    ...
    <div className="space-y-6 pb-20 max-w-md mx-auto">
      ...
      <div>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-amber-50 tracking-tight font-sans">
            Taqwa Streak
          </h1>
        </div>
        <p className="text-xs text-emerald-200/80 mt-1 font-sans">
          Every clean day is an act of worship, akhi.
        </p>
      </div>

      {/* Hero Streak Card (Classic Islamic Emerald & Gold) */}
      <div className="rounded-3xl bg-gradient-to-b from-[#0B3A26] via-[#08281B] to-[#04160E] border border-amber-500/40 p-6 sm:p-7 flex flex-col items-center text-center shadow-[0_12px_45px_rgba(4,20,14,0.7)] relative overflow-hidden">
        {/* Subtle luminous top radiance */}
        <div className="absolute -top-12 inset-x-0 h-32 bg-amber-400/10 blur-3xl pointer-events-none"></div>

        {/* Circular Seedling Sprout Container */}
        <div className="relative w-20 h-20 rounded-full bg-[#0D442D]/80 border border-amber-500/30 flex items-center justify-center mb-3 shadow-inner">
          <div className="w-12 h-12 rounded-full bg-[#082A1C]/90 flex items-center justify-center">
            <Sprout className="w-8 h-8 text-emerald-400 stroke-[2.4]" />
          </div>
        </div>

        {/* Tracked Label */}
        <span className="text-[11px] font-bold uppercase tracking-[0.22em] text-amber-300 font-metric">
          CURRENT STREAK
        </span>
        ...
        <span className="text-sm font-medium text-emerald-200/90 font-metric">
          {currentStreak === 1 ? 'clean day' : 'clean days'}
        </span>

        {/* Achievement Pill Badge with Verified Seal */}
        <div className="mt-3.5 flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-[#04160E]/90 border border-amber-500/35 text-white text-xs font-semibold shadow-sm">
          <span className="text-amber-100">{currentAchievement} Achieved</span>
        ...
        <div className="w-full mt-4 pt-4 border-t border-amber-500/20 flex justify-center">
          <button
            ...
            className={`text-xs flex items-center gap-1.5 font-medium transition-all select-none ${
              hasSealedNiyyahToday
                ...
                : 'text-amber-200 hover:text-white active:scale-95 cursor-pointer'
            }`}
            type="button"
          >
            {hasSealedNiyyahToday ? (
      ...
      {/* Next Milestone Card */}
      <div className="rounded-2xl bg-[#061D14] border border-amber-500/25 p-4 sm:p-5 flex flex-col gap-3 shadow-lg">
        {/* Top Header Row */}
        <div className="flex items-center justify-between">
          ...
          <div className="bg-[#04160E] border border-amber-500/30 text-[10px] font-metric font-semibold text-emerald-200 px-2.5 py-0.5 rounded-full">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            {/* Squircle Icon */}
            <div className="w-12 h-12 rounded-xl bg-[#08261A] border border-amber-500/30 flex items-center justify-center shrink-0 text-xl shadow-inner">
            ...
            <div className="min-w-0">
              <div className="flex items-baseline gap-1.5">
                ...
                <span className="text-xs text-amber-300 font-metric font-semibold">
                  {nextMilestone.days}d
                </span>
              </div>
              <p className="text-xs text-emerald-200/80 mt-0.5 leading-snug line-clamp-1">
                {nextMilestone.line}
              </p>
            </div>
          </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between px-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-amber-50 font-sans tracking-tight">
              All Milestones
            </span>
          </div>
            ...
            return (
              <div
                ...
                className={`p-3.5 rounded-2xl border flex items-center justify-between gap-3 transition-all ${
                  isUnlocked
                    ? 'bg-[#08261A] border-amber-500/40 border-l-4 border-l-amber-400 shadow-md'
                    : 'bg-[#061D14]/90 border-amber-500/20 opacity-80'
                }`}
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div
                    className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 border ${
                      isUnlocked
                        ? 'bg-[#0B3322] border-amber-500/40 text-emerald-400'
                        : 'bg-[#04160E] border-amber-500/20 text-emerald-400/50'
                    }`}
                  >
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {isUnlocked && (
                        <span className="bg-[#0B3322] text-amber-300 border border-amber-500/40 text-[9px] font-extrabold uppercase px-1.5 py-0.5 rounded font-metric tracking-wider">
                          UNLOCKED
                        </span>
                      )}
                    </div>
                    <p className="text-[11px] text-emerald-200/80 leading-normal mt-0.5 line-clamp-1">
                      {m.line}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-xs font-metric font-semibold text-amber-300">
                    {m.days}d
                  </span>
      ...
      <div className="rounded-2xl bg-[#061D14] border border-amber-500/35 p-6 text-center flex flex-col items-center shadow-lg">
        {/* Book Icon Circular Badge */}
        <div className="w-9 h-9 rounded-xl bg-[#08261A] border border-amber-500/35 flex items-center justify-center text-amber-300 mb-2.5">
          <BookOpen className="w-4 h-4 text-amber-300" />
        </div>

        {/* Header Eyebrow */}
        <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-amber-400 font-metric mb-2">
          DIVINE PROMISE
        </span>
        ...
        {/* English Translation */}
        <p className="text-xs text-emerald-100/90 italic font-sans mt-1">
          “He who purifies it will succeed.”
        </p>

        {/* Reference Pill */}
        <div className="mt-3 inline-block px-3 py-1 rounded-full bg-[#04160E] border border-amber-500/30 text-[10px] font-metric font-semibold text-amber-300">
