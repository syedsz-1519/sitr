    ...
    <div className="space-y-6 pb-12">
      ...
      <div>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-bold text-white">Sessions Log & Firewall</h1>
        </div>
      ...
      <div className="rounded-2xl bg-[#170F26] border border-purple-900/30 p-4 flex items-center justify-between gap-3">
        <div>
          <span className="text-xs font-bold text-white block">Test Interception Engine</span>
        ...
        <button
          ...
          className="px-4 py-2 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white text-xs font-semibold uppercase font-metric tracking-wider flex items-center gap-1.5 shadow-md"
        >
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between px-1">
          ...
          <span className="text-[10px] text-emerald-400 font-metric">
            100% Intercept Rate
          </span>
        </div>
        ...
        <div className="space-y-2">
          {events.map((ev) => (
            <div
              key={ev.id}
              className="p-3.5 rounded-2xl bg-[#170F26] border border-purple-900/30 flex items-center justify-between gap-3"
            >
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-9 h-9 rounded-xl bg-purple-950/80 border border-purple-800/40 flex items-center justify-center text-purple-300 shrink-0">
                  {ev.type === 'app' ? (
              ...
              <div className="text-right shrink-0">
                <span className="text-xs font-bold text-purple-300 font-metric block">
                  {formatTimestamp(ev.blockedAt)}
                </span>
                <span className="text-[10px] text-purple-400/70 font-metric">
                  +10m saved
                </span>
              </div>
            </div>
